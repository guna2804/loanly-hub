...
const upcomingPayments = [
  { id: 1, name: "Alex Johnson", amount: 500, dueDate: "2024-02-15", type: "lending", status: "due" },
  { id: 2, name: "Sarah Mills", amount: 1200, dueDate: "2024-02-18", type: "borrowing", status: "upcoming" },
  { id: 3, name: "Mike Chen", amount: 750, dueDate: "2024-02-20", type: "lending", status: "overdue" },
  { id: 4, name: "Emma Davis", amount: 300, dueDate: "2024-02-22", type: "lending", status: "upcoming" }
];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Track your lending and borrowing activities</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 bg-gradient-primary border-0 text-primary-foreground shadow-financial">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-primary-foreground/80 text-sm font-medium">Total Lent</p>
              <p className="text-2xl font-bold">{formatCurrency(summaryData.totalLent, summaryData.currency)}</p>
            </div>
        ...
        <Card className="p-6 bg-gradient-secondary border-0 text-secondary-foreground shadow-success">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-secondary-foreground/80 text-sm font-medium">Total Borrowed</p>
              <p className="text-2xl font-bold">{formatCurrency(summaryData.totalBorrowed, summaryData.currency)}</p>
            </div>
        ...
        <Card className="p-6 bg-gradient-accent border-0 text-accent-foreground shadow-glow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-accent-foreground/80 text-sm font-medium">Interest Earned</p>
              <p className="text-2xl font-bold">{formatCurrency(summaryData.interestEarned, summaryData.currency)}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div key={payment.id} className="flex items-center justify-between p-3 rounded-lg border border-border hover:bg-muted/50 transition-colors">
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full ${
                    payment.status === "overdue" ? "bg-destructive" :
                    payment.status === "due" ? "bg-warning" : "bg-secondary"
                  }`}></div>
                  <div>
                ...
                <div className="text-right">
                  <p className="font-semibold">{formatCurrency(payment.amount, summaryData.currency)}</p>
        ...
        {/* Status Distribution */}
        <Card className="p-6">
          <h2 className="text-xl font-semibold mb-4">Loan Status Distribution</h2>
